...
class CNN(nn.Module):
    ...
    def _create_cnn_layers(self) -> nn.Sequential:
        ...
        for in_ch, out_ch, kernel_size in zip(self._cnn_in_channels, self._cnn_out_channels, self._cnn_kernel_sizes):
            layers.append(nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=kernel_size, padding=(0, (self._n_feats-1)//2)))
    ...
    def _get_fc_indims(self) -> List[int]:
        dummy_data = torch.randn(5, self._in_channel, self._timeseries_len, self._n_feats)
